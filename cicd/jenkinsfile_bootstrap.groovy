...
def loadCIConfigFile() {
    try{
        configFileProvider([configFile(fileId: "cloud_code", targetLocation: 'cicd/')]) {
            load "cicd/cloud_code"
        }
    }catch(Exception e){
...
//def checkEnv(){
//    loadCIConfigFile()
//    String ENV = sh(returnStdout: true, script: 'env')
//    writeFile(file: "./env.groovy", text: "${ENV}", encoding: "UTF-8")
//    String content1 = readFile(file: "./env.groovy")
//    echo "myKeys = ${content1}"
//}
...
//def loadCIConfigFile() {
//    configFileProvider([configFile(fileId: "vbcs-webapp-config", targetLocation: 'cicd/')]) {
//        load "cicd/vbcs-webapp-config"
//    }
//}
...
def updateGitlabPushComment(buildIcon, buildResultStr, lastCommitShortName) {
    ...
    echo "${requestBody}"
    withCredentials([usernamePassword(credentialsId: ${env.credentialsId}, usernameVariable: 'username',
              passwordVariable: 'password')]){
