...
def unitTestAndCodeCoverage(buildType){
	...
	try {
		...
		jacoco([
			classPattern: 'target/classes/com/example/testapi',
			sourcePattern: 'src/main/java'
		])
		def coverageResultStrComment = "<b>Coverage Test Result:</b> <br/><br/>"
		def coverageInfoXmlStr = readFile "target/site/jacoco-aggregate/jacoco.xml"
...
def buildProject(buildType) {
	...
	withCredentials([usernamePassword(credentialsId: "${env.secretHarbor}", usernameVariable: 'username',
	...
	sh """
			sh cicd/dev-build-script.sh ${env.imageName} ${version}
        """
}
...
def release2k8s(buildType){
	...
	def version = "${pomVersion}_u${BUILD_NUMBER}"
    sh """
		sh cicd/dev-deploy-script.sh ${version}
    """
}
...
def buildPushCommit() {
	...
	testingTask['6.1 Automations testing in Test'] = {
		stage("6.1 Automations testing in Test") {
			node("${env.node_slave}") {
				if(env.jobAutoTestStaging == ""){
					echo "skip automations test"
				} else {
					sleep(90)
					build job: "${env.jobAutoTestStaging}"
				}
			}
		}
	}

	testingTask['6.2 Run Security Test'] = {
		stage("6.2 Run Security Test"){
			node("${env.node_slave}") {
				if(env.jobAutoSecurity == ""){
					echo "skip Security test"
				} else {
					sleep(10)
	...
	testingTask['6.3 Run AutoPerfomance Test'] = {
		stage("6.3 Run AutoPerfomance Test"){
			node("${env.node_slave}") {
				if(env.jobAutoPerform == ""){
					echo "skip AutoPerfomance test"
				} else {
					sleep(90)
	...
	testingTask.failFast = true
	//parallel testingTask
